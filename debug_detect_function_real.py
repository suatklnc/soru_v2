...
def debug_detect_function_real(pdf_path, page_num):
    """Gerçek detect_questions_on_page fonksiyonunu debug eder"""
    
    doc = fitz.open(pdf_path)
    page = doc.load_page(page_num)
    
    print(f"=== SAYFA {page_num + 1} GERÇEK DETECT FUNCTION DEBUG ===")
    ...
    print("=== GERÇEK DETECT FUNCTION SIMULASYONU ===")
    ...
    for line_num, line in enumerate(lines):
        ...
        if question_number:
            ...
            if is_instruction:
                print(f"  -> ❌ TALİMAT OLARAK ATLANACAK (continue)")
                continue
    ...
    print(f"\n=== SONUÇ ===")
    print(f"detect_questions_on_page döndürdüğü soru sayısı: {len(questions)}")
    for q in questions:
...
if __name__ == "__main__":
    ...
    debug_detect_function_real("bolunmus_pdf.pdf", 0)
